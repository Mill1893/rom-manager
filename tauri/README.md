...
The nine commands in `src/main.rs`, and nothing else. No Tauri plugin is
depended on — not `fs`, `sql`, `shell`, or `http` — so the capability file in
`capabilities/` is withholding permissions from code that was never compiled in.
A permission cannot be re-granted by editing JSON.
