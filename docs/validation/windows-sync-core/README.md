...
- ~~**Any CI result.**~~ **Superseded on 2026-08-07.** This said GitHub Actions had never executed on this repository, `actions/runs` reporting zero across all branches, and that Windows was type-checked and never run. Both halves are now out of date. CI runs on every push; run [`31221804858`](https://github.com/Mill1893/rom-manager/actions/runs/31221804858) on `main` at `af857fb` is green across `test (ubuntu-latest)`, `test (windows-latest)`, `host-behaviour`, `package-linux` and `package-windows`, and the suite **executes natively on Windows Server 2025** — 503 of the 513 tests, the other ten being `cfg(unix)`.

  Read that narrowly. It moves the suite from type-checked to executed on Windows. It does **not** clear the first bullet above: `tests/confinement.rs` is `#![cfg(unix)]` in its entirety, so the `NtCreateFile` plus `OBJ_DONT_REPARSE` walk still has no test that executes it anywhere, on CI or otherwise. A green Windows job is not evidence for the one Windows claim this document most wants evidence for.
